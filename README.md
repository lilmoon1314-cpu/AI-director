...
## 目标
- 第1批（当前）：构建一个可用的图数据库 + 可视化前端，支持实体与关系的 CRUD、视角过滤、资产上传、Markdown 同步、实体选择器和简单的前期 Agent 对话。
...
## 资产上传与查看
每个实体可上传图片、文档等资产，文件保存至 data/assets/，路径记录在实体 properties.assets 中。前端在节点详情中展示缩略图和文件链接。

## Markdown 导入导出
实体数据可导出为 Markdown 文件（含 YAML front matter 或表格），用户可离线编辑后导入，系统解析并更新数据库，支持冲突检测。
