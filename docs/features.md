...
| F07 | @ 实体选择器       | 输入 @ 触发防抖检索，选择插入引用，提示该实体对当前视角是否可见                                                                                               | 是（前后端联调）                                 | L1: `pytest tests/unit/test_entities_service.py`；L1/L2 前端: `pnpm test:unit && pnpm test:integration`；L3: Playwright `pnpm test:e2e`（场景含 @ 检索 entity-picker.spec）                                                                                       | active |
