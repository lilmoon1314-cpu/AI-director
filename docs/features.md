...
| F12 | 工作台导航与资产页重构 | 资产页两级钻取（7 类型库墙→类型卡片墙→实体 HTML）+ 分区独立搜索 + 空状态引导 + 通用资产表单 modal 化 + 卡片规范统一与 testid 迁移（DESIGN.md §5.3/§9/§10） | 是（前后端联调） | 前端: `pnpm test:unit && pnpm test:integration && pnpm test:e2e`；后端回归: `pytest tests/integration -k asset` | not\_started |
