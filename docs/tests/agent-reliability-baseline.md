...
本文件只记录本任务的验证依据，不代替产品验收状态。六个可执行故障仍未修复；
`xfail` 是明确的未满足要求，不计入通过数。内部契约尚未接入运行链路。
...
两连接同版本 CAS、双 approve/approve-reject 竞争仍待 C 验证。
...
后续修复时将相关 xfail 转为普通回归断言，并补齐该切片的边界场景；
例如 B 还要验证历史以外的摘要/文档/错误入口、emoji、大 schema 与工具后续调用预算。
仅让本文件的六个用例通过不代表 A–H 整体计划验收完成。
