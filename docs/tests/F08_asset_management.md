...
> F12 回注：资产页重构后 IF2「整区替换表单」语义失效（modal 化）、IF3「项目资产平铺卡片」迁移为两级钻取路径——两用例的现行形态由 docs/tests/F12_workbench_navigation_assets.md 的 FI8/FI9 承接并保持断言等价；IF1/IF4 在 F12 测试文件中作为保留回归继续执行。
