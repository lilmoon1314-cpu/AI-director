...
- **判杀器构成（层级覆盖原则，F04 教训 E04 固化）**：判杀测试集合必须覆盖该功能 DoD 的全部「必须」测试层级——判杀器缺哪一层，那一层语义的变异就存在漏杀盲区：
    - **L1 单元（恒为基线，E19 固化 2026-09-09）**：`tests/unit/test_<module>_*.py` **全部文件**（默认判杀器即按此 glob 收敛，且恒为基线——显式传入的 L2/L3/架构路径做并集叠加，不可把单测排除出判杀器）；模块 L1 面常跨多文件（agent 有 tools/llm/prompts/service/memory_docs 五件），缺哪个文件哪片逻辑就是漏杀盲区——F14 首轮只带 2/5 个 agent 单测文件，tools.py/service.py kill rate 掉至 49%/61%。负责杀过滤规则/校验/投影等纯逻辑变异；
