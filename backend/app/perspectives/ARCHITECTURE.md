...
- 项目维度过滤（F11 已落地）：`GET /api/graph?project_id=` 按项目过滤（与视角过滤正交叠加；缺省=默认项目，幽灵项目 404）
...
- 依赖：core、entities（search + get_many，service 层调用）、relations（get_all，service 层调用）、projects（F11：项目存在性校验）
