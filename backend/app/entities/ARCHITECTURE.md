...
F11 已落地：`project_id`（FK projects + 索引）经 Alembic 迁移（存量打包默认项目）；创建缺省归属默认项目并经 ensure_exists 校验，检索/聚合接口支持 project_id 过滤（列表无参=管理面全库，作者工具语义）。

## 依赖

- 依赖：core；projects（F11：归属校验与计数器维护经 projects.service）
