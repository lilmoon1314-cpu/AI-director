...
- 必须（F11 已落地，DESIGN.md §8.5）：projects 业务层零领域模块依赖（import-linter 契约），级联删除仅在 projects.router 组合层编排；领域模块对项目的引用一律经 `projects.service` 归属校验。
- 必须（F11 已落地，DESIGN.md §8.2）：删除项目必须显式级联清理——主库事务删除该项目实体/关系后，按项目实体集合清扫 assets.db 的资产记录 / 图片元数据 / 物理文件；清扫失败的补偿边界 = 既有读取时孤儿清扫兜底（跨库两事务，DECISIONS 2026-09-06）。
...
- 必须（F11 已落地，DESIGN.md §8.1）：`projects` 表与 `entities` / `relationships`.`project_id`（FK + 索引）经 Alembic 迁移落地，存量数据打包默认项目（向后兼容）；展示面（/api/graph、/api/assets/entities）缺省归属默认项目并按 project 维度过滤，写路径经 ensure_exists 归属校验；entities/relations 列表端点无参 = 作者管理面全库语义（F06 决策延伸，@ 检索/运维清理用途），前端项目内调用恒显式携带 project_id。
